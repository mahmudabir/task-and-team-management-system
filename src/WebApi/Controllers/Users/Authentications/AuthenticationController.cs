    ...
    : ControllerBase
{
    ...
    private async Task<ActionResult<TokenResponse>> Login(TokenRequest request)
    {
        ...
        if (signInResult.Succeeded)
        {
            ...
            string accessToken = GenerateJwtTokenAsync(user, roles);
    ...
    public async Task<IActionResult> Register([FromBody] User userRegistration, CancellationToken ct = default)
    {
        ...
        var roleResult = await userManager.AddToRolesAsync(identityUser, userRegistration.Roles);
    ...
    [NonAction]
    public string GenerateJwtTokenAsync(ApplicationUser user, IList<string> roles)
    {
        return GenerateJwtToken(user, roles, jwtOptions.CurrentValue.Key, jwtOptions.CurrentValue.Issuer, jwtOptions.CurrentValue.Audience, _accessTokenExpiresIn);
    }

    private static string GenerateJwtToken(ApplicationUser user, IList<string> roles, string? signingKey, string? issuer, string? audience, int accessTokenExpiresIn)
    {
